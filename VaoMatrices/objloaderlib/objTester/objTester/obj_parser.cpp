...
obj_vector* obj_parse_vector(bool search2)
{
...
/*		
	...
	while( fgets(current_line, OBJ_LINE_SIZE, obj_file_stream) )
	{
		...
		{
            list_add_item(&growable_data->vertex_list,  obj_parse_vector(true), NULL);
		}
		
		else if( strequal(current_token, "vn") ) //process vertex normal
		{
            list_add_item(&growable_data->vertex_normal_list,  obj_parse_vector(true), NULL);
		}
		
		else if( strequal(current_token, "vt") ) //process vertex texture
		{
            search2=false;
            list_add_item(&growable_data->vertex_texture_list,  obj_parse_vector(search2), NULL);
		}
